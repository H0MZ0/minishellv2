...
/*   Updated: 2025/05/30 18:46:21 by sjoukni          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	handle_heredoc_wrap(char *target, char *delim, t_list *alloc_list,
		t_shell *shell)
{
	if (!handle_heredoc(shell->cmds, target, delim, alloc_list))
	{
...
// 		return (handle_redir_in(cmd, target, alloc_list));
// 	else if (token->type == REDIR_OUT || token->type == APPEND)
// 		return (handle_redir_out(cmd, token->type, target, alloc_list));
// 	else if (token->type == HEREDOC)
// 		return (handle_heredoc_wrap(target, token->next->value, alloc_list,
// 				shell));
// 	*current = token->next;
// 	return (1);
// }
